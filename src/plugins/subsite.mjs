...
const buildSubsitePlugin = async (buildSite, target) => {
  const subsite = async function (fastify, opts, done) {
    ...
    const siteServices = buildSiteServices
      ? await buildSiteServices(_yx, settings, site, root)
      : {}
    ...
    fastify.decorateRequest('_yx', null)
    ...
    enhance && (await enhance(fastify, yxConfig))
    ...
    if (buildSite && (target === '*' || target === site)) {
      ...
      prebuild && (await prebuild(siteRoot, site, yxConfig, fileRoutingTable))

      const _build =
        (customBuildSite && customBuildSite.build) ||
        (defaultBuildSite && defaultBuildSite.build)

      _build && (await _build(siteRoot, site, yxConfig, fileRoutingTable))

      const postbuild =
        (customBuildSite && customBuildSite.postbuild) ||
        (defaultBuildSite && defaultBuildSite.postbuild)

      postbuild && (await postbuild(siteRoot, site, yxConfig, fileRoutingTable))
    }
