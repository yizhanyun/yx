...
Now `yx` could be interprted as `Ying Xiong` server , or in Chinese `英雄(Ying Xiong, meaning hero)`
...
和Next.js一样, yx从 `<site>/pages` 目录渲染和发送页面. 和 Next.js 不同, 英雄yx支持静态 HTML 文件，也支持渲染模板引擎文件，例如 [liquidjs](https://github.com/harttle/liquidjs).
...
英雄yx使用5000作为默认端口。可以修改`settings.mjs`设置不同端口。
...
目前 英雄 yx 只使用 `defaultSite`, `lang` 与 `port`.
...
英雄 yx 对子站点不做过多假设，只有三个简单的目录结构要求和一个保留文件后缀名。

- <site-root>/pages : 用于html / 模板文件等
- <site-root>/public/static: 用于不需要额外处理的静态文件
- <site-root>/public/bundle: 用于使用打包工具或编译工具生成静态文件
- <site-root>/pages/**/<filename>.boot.mjs:`.boot.mjs`文件扩展用作为模板预加载数据。 英雄 yx不会从`pages`目录发送该后缀的文件，但会从 `static` 与 `bundle` 目录发送该后缀的文件。
...
为了支撑英雄点，支持定制，支持全局以及子站点强化服务器，英雄采用结构化启动流程准备服务器：
...
英雄 yx 后续将逐步为典型的框架和库增加预设置的项目模板，为您节省时间。
...
英雄的一个目标是方便实验不同的HTML view引擎 / 模板引擎。
...
当 英雄 yx 渲染一个html模板时，比如 `index.liquid`, 会首先在同一目录下寻找 `index.liquid.boot.mjs`文件。该文件应该export提供一个名为`getServerProps` 的异步函数，该函数有如下签名：
...
英雄yx booter将会 require文件 `<root>/src/enhancer.mjs` 获得强化器函数，该函数应该有如下签名：
...
英雄yx booter将会require文件 `<root>/sites/<subsite>/src/enhancer.mjs` 获得子站点强化器函数，该函数有如下签名：
...
英雄提供一组默认模板/view引擎。开发者可以提供自己的引擎。
...
有时服务器需要将全局服务例如数据库连接发送到每个子站点服务器。英雄yx将require本文件`<root>/src/globalServices.mjs`, 该文件应该export默认 `buildGlobalServices` 函数。 函数签名应为:
...
## 为什么选择Fastify为基础开发英雄 yx

当然因为我们用过Fastify，而且也觉得Fastify不错，不过更主要是因为它的一些很不错的功能。

### rewriteUrl

 rewriteUrl 让英雄yx可以把其一个类似于 `my-site-in-ejs.localhost/index.ejs` 的请求重写为 `localhost/my-site-in-ejs/index.ejs`. 这个功能结合下面描述的支持前缀的plugin功能，让英雄yx可以就像没有子站点一样，方便的开发路由器与handler。
...
这部分记录yx开发中的重要的设计，想法，理念与选择决定。 由于英雄 yx正在早期阶段，本部分不追求完备性和良好的结构，而是及时反应开发中设计理念和决定。
...
英雄yx 规定 以 `/static/`或`/bundle/` 开始的url，都指向静态文件，直接发送，不会进行任何解析处理。
...
i18n 通过字典方式实现，每个key对应的值为字符串，或返回字符串的函数。英雄服务器运行消息i18n的目录为：
